...
max(a, b);            // 最大值
min(a, b);            // 最小值
pow(2, 10);           // 2^10 = 1024.0（返回 double）
sqrt(16);             // 4.0
Random().nextInt(100); // 0..99 随机整数
Random().nextDouble(); // 0.0..1.0 随机小数
sin(pi / 2);          // 1.0（参数是弧度）
cos(0);               // 1.0
exp(1);               // e^1 = 2.718...
log(e);               // ln(e) = 1.0
Point(3, 4).distanceTo(Point(6, 8));  // 5.0
```
...
|------|----------|---------|
| 最大值 | `max(a, b)` | 找出最高分 |
| 随机数 | `Random().nextInt(n)` | 抽奖、随机密码 |
| 打乱顺序 | `list.shuffle(Random())` | 洗牌、随机播放 |
| 平方根 | `sqrt(x)` | 计算距离、标准差 |
| 幂运算 | `pow(x, y)` | 复利计算、面积体积 |
| 三角函数 | `sin`/`cos`/`tan` | 动画、游戏物理 |
| 对数 | `log(x)` | 数据缩放、复杂度分析 |
| 取整 | `ceil`/`floor`/`round`/`truncate` | 价格计算、分页 |

**实用技巧：**
- `Random(seed)` 固定种子 → 可复现的随机，方便测试
- 角度转弧度：`deg * pi / 180`，弧度转角度：`rad * 180 / pi`
- `log(x) / log(n)` = 以 n 为底的对数
- `pow` 返回 `double`，要整数用 `.toInt()`
- `Random` 是伪随机，**不适合加密**
...
| 课 | 方式 | 做法 |
|---|------|------|
| 71 | dart:io HttpServer | `await for` + if-else 手动分发 |
| 72 | shelf Handler | 一个 `Request → Response` 函数 |
| 73 | shelf_router | 声明式 `.get()/.post()` 自动匹配 |
| 75 | Cascade | 串联多个处理器，逐个尝试 |
...
71课: 服务器只处理请求，全部手写
      ↓
72课: 引入 Handler 抽象，serve() 替代 HttpServer.bind()
      ↓
73课: 引入 Router，替代 if-else 路由分发
      ↓
74课: 统一 JSON 响应格式，RESTful 设计
      ↓
75课: Cascade 串联 API + 静态文件
      ↓
76课: Pipeline + Middleware 洋葱模型
      ↓
77课: 数据库持久化，数据重启不丢
```
...
### 项目依赖
```bash
dart pub add shelf                    # HTTP 框架
dart pub add shelf_router             # 路由
dart pub add shelf_static             # 静态文件服务
dart pub add sembast                  # NoSQL 数据库
```

### 日志规范
```dart
final log = File('server.log').openWrite(mode: FileMode.append);
log.writeln('[${DateTime.now()}] 消息');
await log.flush();  // flush() = 强制写入硬盘，防止丢日志
```
